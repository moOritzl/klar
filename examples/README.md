...
draw: *Sozial* stays above 50 % of alcohol contexts, and the last two weeks are
set so the trend card reads "↓" instead of "↑". See `plan_alcohol` in the
generator.

## One thing happens on the first launch after importing

**Der Morgen danach** asks about the newest alcohol evening, if it ended less
than 48 h before the reference day. In the shipped file the newest evening is
well over 48 h before the reference day (the alcohol curve tapers off before
`--today`), so the card does not actually appear — regenerate with a `--today`
close to the newest logged evening to see it on import.

It is one-shot: answering it consumes it.
...
failing loudly if a quota is already spent, a month has no entry-free days, fewer
than three morning-after records are answered, or the newest alcohol evening is
not the one left open.
